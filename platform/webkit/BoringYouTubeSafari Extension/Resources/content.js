// Boring YouTube - Safari Content Script
...
(function() {
    ...
    // Get extension state from Safari extension
    function getExtensionState(retryCount = 0) {
        // Safari extension messaging
        if (typeof safari !== 'undefined' && safari.extension) {
            safari.extension.dispatchMessage('getState');
        } else {
            // Fallback for when Safari APIs are not available
            console.log('Safari APIs not available, defaulting to enabled');
    ...
    function init() {
        ...
        console.log('Boring YouTube Safari extension loaded');
    }
    
    // Listen for messages from Safari extension
    if (typeof safari !== 'undefined' && safari.extension) {
        safari.extension.addEventListener('message', function(event) {
            if (event.name === 'toggleExtension') {
                toggleExtension(event.message.enabled);
            } else if (event.name === 'extensionState') {
                toggleExtension(event.message.enabled);
            }
        }, false);
    }
